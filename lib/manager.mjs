...
export function reportMarkdown(report) {
  ...
  // Operators whose product PAGES the WAF refused. Not silence — they scrape, and then shed
  // draws, because on most WooCommerce themes the ticket cap and close date exist only in the
  // page body. Invisible until now: the loss surfaced as "required: missing total_entries",
  // i.e. as our parser's fault. 420 draws in one day on 8 Sep 2026, against zero for the same
  // operators read from a residential IP. Reported apart from silence because the remedy
  // differs — a different egress IP, not a selector.
  //
  // The HEADLINE is draws lost, never pages refused. They are not the same number: the API
  // description sometimes carries a labelled cap, and the woo zap ajax fills cap+date for
  // whole operators, so an operator can have every page refused and lose nothing. Counting
  // pages as losses would overstate the damage and send topup after operators that are fine.
  // Zero-loss operators are still listed, so the contrast is visible; the section stays away
  // entirely when nothing was actually lost.
  const blocked = (perOperator || []).filter((c) => c.pageBlocks?.blocked > 0);
  const lost = blocked.reduce((a, c) => a + (c.pageBlocks.starved || 0), 0);
  if (lost > 0) {
    const hurt = blocked.filter((c) => c.pageBlocks.starved > 0);
    md += `⛔ **Product pages refused — ${lost} draw(s) left without a cap or date, across ${hurt.length} operator(s)**\n\n`;
    md += `The listing API answered; the pages carrying the ticket cap and close date did not. The gate drops these as \`missing total_entries\` / \`missing draw_date\`, which reads as a parser fault and is not one — golf-star lost 32 draws this way on the runner and 0 from a residential IP. Counted as co-occurrence (page refused AND field missing), so treat it as an upper bound. \`bun topup.mjs\` re-scrapes these from a residential IP.\n\n`;
    for (const c of blocked.sort((a, b) => (b.pageBlocks.starved || 0) - (a.pageBlocks.starved || 0)).slice(0, 20)) {
      const t = c.pageBlocks;
      const causes = Object.entries(t.causes || {}).sort((a, b) => b[1] - a[1]).map(([k, n]) => `${k}×${n}`).join(", ");
      const verdict = t.starved ? `**${t.starved} draw(s) left short**` : `no draw left short`;
      md += `- \`${c.slug}\` — ${verdict} · ${t.blocked} of ${t.ok + t.blocked} pages refused (${causes})\n`;
    }
    md += `\n`;
  }
