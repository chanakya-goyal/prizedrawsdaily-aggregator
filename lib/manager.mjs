...
export function reportMarkdown(report) {
  ...
  // Operators whose product PAGES the WAF refused, even though their JSON listing answered.
  // This is not silence — they scrape fine and then lose individual draws, because on most
  // WooCommerce themes the ticket cap and close date exist only in the page body. It was
  // invisible until now: the loss surfaced as "required: missing total_entries", i.e. as our
  // parser's fault. 420 draws in one day on 8 Sep 2026, against zero for the same operators
  // read from a residential IP. Reported separately from silence because the remedy differs —
  // a different egress IP, not a selector.
  const blocked = (perOperator || []).filter((c) => c.pageBlocks?.blocked > 0);
  if (blocked.length) {
    const lost = blocked.reduce((a, c) => a + c.pageBlocks.blocked, 0);
    md += `⛔ **Product pages refused — ${lost} page(s) across ${blocked.length} operator(s)**\n\n`;
    md += `The listing API answered, but the pages carrying the ticket cap and close date did not. Draws lost this way are reported by the gate as \`missing total_entries\` / \`missing draw_date\`; the cause is the egress IP, not the parser.\n\n`;
    for (const c of blocked.sort((a, b) => b.pageBlocks.blocked - a.pageBlocks.blocked).slice(0, 15)) {
      const t = c.pageBlocks;
      const causes = Object.entries(t.causes || {}).sort((a, b) => b[1] - a[1]).map(([k, n]) => `${k}×${n}`).join(", ");
      md += `- \`${c.slug}\` — ${t.blocked} of ${t.ok + t.blocked} refused (${causes})\n`;
    }
    md += `\n`;
  }
