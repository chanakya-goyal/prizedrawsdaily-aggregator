...
export function correctionDecision(stored, fresh, { now = new Date() } = {}) {
  ...
  const base = { drift: v.drift, flags: v.flags, fields: Object.keys(v.drift) };
  if (!v.hasDrift) return { correct: false, reason: "no drift", ...base };
  if (v.flags.length) return { correct: false, reason: `fresh read is flagged (${v.flags.join("; ")})`, ...base };
  return { correct: true, reason: `clean read disagrees on ${base.fields.join(", ")}`, ...base };
}
