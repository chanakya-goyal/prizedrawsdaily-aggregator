...
// `evidence` is the SAME payload staleDateDecision takes, and it must be passed: the sweep has
// already read the operator's live data by the time this is asked, and deciding from the stored
// row alone would throw that away. A draft whose stored date has passed but which the operator
// still sells is not dead — the DATE is what is wrong, and staleDateDecision's extend path owns
// it. Ending it would pull a real, still-enterable competition out of the queue. (Caught in
// review on PR #40 before it could become the normal case; the first live run of this rule had
// to be rolled back and redone because of it.)
export function deadDraftDecision(row, evidence = {}, now = new Date()) {
  if (row?.status !== "draft") {
  ...
  // The operator's own flag outranks our stored date, exactly as it does for a live row. Note
  // this is `=== true`, not `!== false`: "unknown" (null, unreachable) is not a reprieve — a
  // draft we cannot read whose own advertised date has passed is the case this rule exists for.
  if (evidence?.purchasable === true) {
    return { end: false, reason: "stored date passed but the operator is still selling it — a date problem, not a dead draw" };
  }
