...
export function staleDateDecision(stored, evidence = {}, now = new Date()) {
  ...
  // Confidence grades the EVIDENCE, not the verdict. woo/shopify expose the operator's own
  // purchasability flag; render offers only a text heuristic; anything else (an api operator
  // the sweep has no adapter path for, or an unknown method) has no evidence at all and must
  // never be able to authorise a write by looking confident.
  const confidence = source === "woo" || source === "shopify" ? "high" : source === "render" ? "medium" : "low";
