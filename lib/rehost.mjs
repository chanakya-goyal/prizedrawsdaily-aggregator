...
import { putObject, publicBases, r2Client, IMAGE_PROVIDER } from "./storage.mjs";
...
// Upload bytes to <provider>/<path>; returns the public URL. Routed through
// putObject so Supabase and R2 share one write path — and one cache-control.
async function upload({ buf, type }, path, { supabaseUrl, serviceKey }) {
  return putObject({ path, bytes: buf, contentType: type, supabaseUrl, serviceKey, bucket: BUCKET });
}

// Is this URL already on storage we own? Must cover EVERY base, not just the
// active one: after the R2 move the database is full of R2 URLs, and a check that
// only knew about Supabase would re-download and re-upload our own images on every
// single pass — burning egress to achieve nothing.
function isOurs(url, creds) {
  return publicBases({ ...creds, bucket: BUCKET }).some((b) => url.startsWith(b));
}

async function remove(path, { supabaseUrl, serviceKey }) {
  try {
    if (IMAGE_PROVIDER === "r2") { await r2Client().delete(path); return; }
...
export async function rehostImage(imageUrl, opSlug, drawSlug, { supabaseUrl, serviceKey }) {
  ...
  if (isOurs(imageUrl, { supabaseUrl })) return { url: imageUrl, via: "own", changed: false }; // already on our storage
