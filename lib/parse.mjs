...
const CAT_RULES = [
  ...
  // "warhammer" in the title, so list them explicitly or they fall through to the cash fallback.
  ...
  // ⚠️ "golf" is NOT listed bare. On a directory that carries a dedicated golf operator (the
  // largest by live inventory) a bare `golf` matches every box of golf balls, every driver and
  // every trolley — and because cars are tested before luxury, all of it read as car draws. A
  // genuine VW Golf is still caught: those titles carry "VW"/"Volkswagen", or "GTI"/"GTD"/
  // "Golf R", all matched below.
...
// is a meaningful answer ("no evidence"), and keeping it distinct from the cash-prizes
// fallback is what lets downstream checks tell "I can't corroborate this" apart from "this is
// wrong". Conflating the two held 241 of 400 drafts out of publication.
...
export function inferCategory(input = {}) {
  return categoryEvidence(input) ?? "cash-prizes"; // generic fallback; manager re-checks
}

// Map an operator's OWN product-category labels (WooCommerce categories / Shopify product_type +
// tags) to one of our six. The operator's taxonomy is authoritative when it's specific (e.g. a
...
const OP_CAT_MAP = [
  ["collectibles", /\b(warhammer|age of sigmar|sigmar|astra militarum|space marines?|necrons?|tyranids?|horus heresy|kill team|games workshop|citadel|lego|pok[eé]mon|trading cards?|lorcana|magic the gathering|\bmtg\b|yu-?gi-?oh|one piece|funko|tcg|collectibles?|model kits?|airbrush(?:es)?|miniatures?)\b/i],
  ...
  ["house-draws", /\b(house draws?|houses?|homes?|propert(?:y|ies)|real estate)\b/i],
  ["luxury", /\b(luxury|watch(?:es)?|jewell?ery|holidays?|travel|spa days?|golf|fishing|designer)\b/i],
...
export function fieldsFromHtml({ html, url, op = {}, knownTitle, knownImage, knownPrice, descriptionText, prizeText, apiCategories, apiStock }) {
  ...
  // whatever the cash fallback gives.
...
// taxonomy → specific title inference → whatever the cash fallback gives.
export function resolveCategory({ op = {}, title, grand_prize, url, apiCategories }) {
  ...
  return op.category
    ...
    || (inferred !== "cash-prizes" ? inferred : null)
    || opCat || inferred;
}
