...
// publication and shelved ~100 non-cash draws (mostly golf gear) under Cash Prizes.
export function categoryEvidence({ title, grand_prize, url } = {}) {
  // neutralise "Van Gogh" so the car keyword "van" can't claim a Pokémon/art prize.
  const hay = `${title || ""} ${grand_prize || ""} ${url || ""}`.replace(/van[\s-]?gogh/ig, "vangogh");
