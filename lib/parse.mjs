...
export function fieldsFromHtml({ html, url, op = {}, knownTitle, knownImage, knownPrice, descriptionText, prizeText, apiCategories, apiStock }) {
  ...
  //   d) the operator's API stock count ("97 in stock") — the real per-product number on JS-bar
  //      sites whose live total is otherwise rendered client-side (BigBeastie, You Could Win)
  //   e) last resort: the noisy whole-page extraction (progress bar / bare "N tickets")
  const apiStockNum = (Number.isFinite(apiStock) && apiStock >= 10 && apiStock <= 10_000_000) ? Math.round(apiStock) : null;
  ...
  const total_entries =
    ...
    ltyCap ??
    apiStockNum ??
    extractEntries(text, op.patterns);
