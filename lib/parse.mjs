...
export function extractEntries(text, opPatterns) {
  ...
  const tier1 = collect([
    ...
    // "Total Entries: N" / "Max Tickets - 12,499" — the WooCommerce Lottery plugin's labelled
    // counter. Mandatory ':' or '-' immediately after the noun keeps "Max Entries per person: 20"
    // (a per-user limit, not the cap) and "total tickets sold: N" from matching.
...
export function fieldsFromHtml({ html, url, op = {}, knownTitle, knownImage, knownPrice, descriptionText }) {
  ...
  // The WooCommerce Lottery plugin renders the close date into a data-enddate="YYYY-MM-DD HH:MM"
  // attribute (the visible countdown reads only "Draw Fri at 12pm"), so textOf() strips it before
  // any regex runs — read it straight off the element.
  const attrDate = $("[data-enddate]").first().attr("data-enddate")
    || $("[data-end-date]").first().attr("data-end-date")
    || $("[data-draw-date]").first().attr("data-draw-date") || null;
  ...
  const draw_date = (attrDate && extractDate(attrDate, op.patterns))
