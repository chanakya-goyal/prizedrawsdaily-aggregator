...
export function extractDate(text, opPatterns) {
  ...
  // 2b) short form: abbreviated month, optional 2-digit year, or no year at all.
  //     Runs after the unambiguous 4-digit-year forms so nothing existing changes, and only a
  //     LABELLED or time-anchored match is trusted ahead of the ISO/numeric tiers below.
  const shortCands = shortDateCandidates(t);
  const strong = shortCands.find((c) => c.score > 0);
  if (strong) { const iso = ymd(strong.year, strong.mo, strong.day, t, strong.index); if (iso) return iso; }
  ...
  // 5) last resort: an unlabelled short date. Weakest signal here, so it only runs once every
  //    stronger form has failed; the business gate still has to accept the resulting window.
...
export function fieldsFromHtml({ html, url, op = {}, knownTitle, knownImage, knownPrice, descriptionText, prizeText, apiCategories, apiStock }) {
  ...
  // Prefer the product summary over the whole page, for the same reason extractEntries does:
  // the related-competitions carousel prints OTHER draws' dates in the identical format, and
  // the short-date tier cannot tell them apart from the real one on text alone.
