...
export function extractEntries(text, opPatterns) {
  ...
  const tier1 = collect([
    /\b(?:max(?:imum)?|total)\s+(?:of\s+)?([\d,]{2,})\s+(?:tickets?|entries?)\b/gi,
...
const CAT_RULES = [
  ...
  ["car-draws", /\b(car|bmw|audi|mercedes|porsche|ford|vw|golf|gtr|m2|m3|m4|rs\d|motorbike|motorcycle|bike|van|campervan|vehicle|supercar|tesla|lamborghini|ferrari|range rover|defender)\b/i],
  ...
  ["luxury", /\b(rolex|omega|watch|jewellery|jewelry|diamond|designer|holiday|getaway|tackle|fishing|carp|pok[eé]mon|tcg|trading card|funko|collectible|handbag|chanel|louis vuitton|gucci)\b/i],
