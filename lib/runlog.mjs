...
// test/page-block.test.mjs round-trips producer → consumer so a reworded line cannot silently
// stop matching, which is exactly how the silent-operator regex broke before.
//
// `lost` is the number that matters and is NOT the same as `blocked`: an operator whose API
// description (or the woo zap ajax) supplied the cap and date loses nothing even with every
// page refused. Retrying those would be pure waste, so the line says which case it is and this
// reads it back rather than inferring it from the page count.
export function pageBlockedSlugs(log, isKnown = () => true) {
  const found = new Map();
  const RX = /⚠️ \[([a-z0-9-]+)\] (\d+) of (\d+) product pages unreadable \([^)]*\) — (?:(\d+) left a draw|no draw left short)/g;
  for (const m of String(log).matchAll(RX)) {
    if (!isKnown(m[1])) continue;
    const row = { slug: m[1], blocked: Number(m[2]), total: Number(m[3]), lost: m[4] ? Number(m[4]) : 0 };
    const prev = found.get(m[1]);
    // Keep the worst reading if an operator somehow appears twice in one log.
    if (!prev || prev.lost < row.lost || (prev.lost === row.lost && prev.blocked < row.blocked)) found.set(m[1], row);
  }
  return [...found.values()].sort((a, b) => b.lost - a.lost || b.blocked - a.blocked);
}
