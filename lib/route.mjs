...
// ---- ticket-cap provenance (spec §10.4) ----
// These three columns exist so the carousel can render an odds figure and defend it: "1 IN
// 13,995" is a published objective claim, and CAP 3.7 wants the evidence held beforehand.
//
// THE ONE RULE: the provenance must always describe the number actually stored in the row.
// A method that describes a different number is a FALSE provenance claim, which is worse
// than no claim at all. So these columns are written in exactly two situations — alongside a
// total_entries we are writing, or when a fresh read re-confirms the stored value — and never
// on their own otherwise.
export function figuresPatch(fresh, now) {
  return {
    figures_source_url: fresh.figures_source_url ?? null,
    figures_checked_at: now.toISOString(),
    total_entries_method: fresh.total_entries_method ?? null,
  };
}

// Re-confirmation: this fetch read the same cap that is already stored. The row's number has
// not moved, so stamping it is truthful and it is the ONLY thing keeping a stable, correct row
// inside the renderer's 48h freshness window. Without this a draw that is right, and stays
// right, quietly ages out of eligibility precisely because nothing about it ever changes.
export const reconfirmsCap = (existing, fresh) =>
  fresh?.total_entries != null && existing?.total_entries === fresh.total_entries;
...
export function routeDraw(existing, fresh, {
  ...
  if (existing.status === "active") {
    // A skip still carries a provenance stamp when the read re-confirmed the stored cap. This
    // is the case that is easy to miss and expensive to miss: nothing is wrong with the row, so
    // nothing is written, so figures_checked_at never moves, so a correct row falls out of the
    // renderer's freshness window for being stable. Computed before the correct-live-off exit
    // so that switching corrections off cannot also switch off freshness.
    const stamp = reconfirmsCap(existing, fresh) ? figuresPatch(fresh, now) : null;
    if (!correctLive) return { kind: "skip", reason: "correct-live-off", tpv, stamp };
    const decision = correctionDecision(existing, fresh, { now });
    if (!decision.correct) return { kind: "skip", reason: "no-correction", decision, tpv, stamp };
    // Checked AFTER the decision, exactly as before, so the same draws consume the cap.
    if (correctRemaining <= 0) return { kind: "skip", reason: "correction-cap", decision, tpv, stamp };
    ...
    // A pool-only drift rewrites no field, so the stored cap is unchanged — stamp it only if
    // this read actually re-confirmed that cap.
    if (stamp) Object.assign(row, stamp);
    if (decision.fields.some((f) => f !== "total_prize_value")) {
      Object.assign(row, {
        ...
        // Written in the same Object.assign as total_entries so the two can never separate.
        ...figuresPatch(fresh, now),
      });
